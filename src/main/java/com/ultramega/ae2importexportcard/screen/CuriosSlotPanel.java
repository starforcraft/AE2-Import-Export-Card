...
public final class CuriosSlotPanel {
    private static final Identifier INVENTORY = makeId("textures/gui/curios/inventory.png");
    ...
    public void render(GuiGraphicsExtractor graphics, Font font, int left, int top, int mouseX, int mouseY) {
        if (!this.open) {
            return;
        }
        ...
        for (int cell = 0; cell < this.visibleSlots; cell++) {
            ...
            UpgradeScreen.drawSlotHighlight(graphics, UpgradeType.EXPORT, font, sx, sy, filter > 0, filter);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button, boolean carrying) {
        ...
        if (rx >= 0 && ry >= 0 && rx < this.columns * 18 && rx % 18 < 16 && ry % 18 < 16) {
            ...
            if (cell < this.visibleSlots && index < this.slots.size()) {
                ...
                ClientPacketDistributor.sendToServer(new CurioSlotUpdateData(this.menu.containerId, key, filter));
                return true;
            }
        }
    ...
    private void drawSlotColumns(GuiGraphicsExtractor graphics) {
        for (int cell = 0; cell < this.visibleSlots; cell++) {
    ...
    private void drawPageButton(GuiGraphicsExtractor graphics, boolean next, int mouseX, int mouseY) {
        ...
        if (hovered) {
            graphics.setTooltipForNextFrame(Component.translatable("gui.curios.page", this.page + 1, this.lastPage() + 1), mouseX, mouseY);
        }
    }
