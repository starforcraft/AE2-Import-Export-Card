...
public final class BlockPickerHandler {
    ...
    public static void handle(final ServerPlayer player, final BlockPos blockPos, final Direction direction) {
        if (player.gameMode.getGameModeForPlayer() != GameType.SURVIVAL || !player.level().isLoaded(blockPos) || !player.canInteractWithBlock(blockPos, 1.0)) {
            return;
        }
        ...
        final ItemStack pickedStack = blockState.getCloneItemStack(hitResult, player.level(), blockPos, player);
